...
void Interpolate::processPoints(const sensor_msgs::msg::PointCloud2::SharedPtr points_xyziradt_msg)
{
  ...
  // 新しいPointCloud2データの作成
  pcl::PointCloud<velodyne_pointcloud::PointXYZIRADT> noise_filtered_points;
  noise_filtered_points.header = points_xyziradt->header;
  noise_filtered_points.reserve(points_xyziradt->size());

  // 新しいPointCloud2データの作成
  // pcl::PointCloud<velodyne_pointcloud::PointXYZIRADT> noise_tmp_points;
  // noise_tmp_points.header = points_xyziradt->header;
  // noise_tmp_points.reserve(points_xyziradt->size());

  for (const auto& point : *interpolate_points_xyziradt) {
    // x, y, z, i情報の取得
    float x = point.x;
    float y = point.y;
    float z = point.z;
    ...
    if (distance <= 2.5 && distance >= 0.0){
      if(z < 1.2 || z > 2.3 || intensity > 20.0){
        noise_filtered_points.push_back(point);
      }
    }
    // ノイズ条件(2.5~13.0m)
    if(distance > 2.5){
      if (distance > 13.0 || z < 0.6 || z > 3.0 || intensity > 2.0) {
        noise_filtered_points.push_back(point);
      }
    }
  ...
  if (velodyne_points_interpolate_pub_->get_subscription_count() > 0) {
    const auto interpolate_points_xyzir = noise_filtered_points;
    auto ros_pc_msg_ptr = std::make_unique<sensor_msgs::msg::PointCloud2>();
    pcl::toROSMsg(interpolate_points_xyzir, *ros_pc_msg_ptr);
    velodyne_points_interpolate_pub_->publish(std::move(ros_pc_msg_ptr));
  }
