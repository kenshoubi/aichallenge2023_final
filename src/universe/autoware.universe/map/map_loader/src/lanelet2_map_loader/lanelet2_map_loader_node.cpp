...
: Node("lanelet2_map_loader", options)
{
  ...
  bool flag = false;
  lanelet::utils::overwriteLaneletsCenterline(map, center_line_resolution, flag);
  if(flag)RCLCPP_ERROR(rclcpp::get_logger("map_loader"), "aaaaaaaaaaflag is true");
  RCLCPP_ERROR(rclcpp::get_logger("map_loader"), "aaaaaaaaaaaaaaaaalanelet2_map_projector_type is not supported");
  // create map bin msg
