...
/*********my_add*******/
  RCLCPP_INFO(this->get_logger(), "aaaaaaaa%ld", map_marker_array.markers.size());
  int num = 0;
  for(size_t i=0; i < map_marker_array.markers.size(); i++){
    if(map_marker_array.markers[i].ns == "center_lane_line")num+=(map_marker_array.markers[i].points.size());
  }
  RCLCPP_INFO(this->get_logger(), "aaaaaaaa%d", num);
/***********************/
