  ...
  const bool use_resampling, const double input_points_interval) const
{
  ...
  for (size_t i = 0; i < output.size(); ++i) {
    ...
    v_curvature_max = std::max(v_curvature_max,base_param_.min_curve_velocity); //honntou ha std::max
    RCLCPP_DEBUG(logger_, "--------------------my_test1 %lf --------------------",v_curvature_max);
    RCLCPP_DEBUG(logger_, "--------------------my_test2 %lf --------------------",base_param_.min_curve_velocity);
    /*******add**/
    //if (curvature >= 0.000) {
    //  v_curvature_max = 0.2788;
    //}
    //koko
    /**************/
    if (output.at(i).longitudinal_velocity_mps > v_curvature_max) { //add && false
