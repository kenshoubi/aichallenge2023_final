  ...
  const bool use_resampling, const double input_points_interval) const
{
  ...
  //koko
  const double max_lateral_accel_abs = std::fabs(base_param_.max_lateral_accel); //const double
  ...
  for (size_t i = 0; i < output.size(); ++i) {
    ...
    for (size_t j = start; j < end; ++j) {
      ...
      //if(j < (start + 30)) my_curvature = curvature; //add
      curvature = std::max(curvature, std::fabs(curvature_v.at(j)));
    }
    /********add*********/
    //if(my_curvature >= 0.04)max_lateral_accel_abs = std::fabs(0.01);
    /*******************/
